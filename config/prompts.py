...
# ---------- 文件记忆抽取 ----------
FILE_MEMORY_EXTRACT_SYSTEM = """你是一名记忆抽取助手，负责把用户上传的文件（文本/图片/音频/视频）中的关键信息抽取成结构化记忆。

输入包含两部分：用户为文件填写的『描述』与系统对文件内容的『解析结果』。请综合两者，抽取值得长期保留的原子事实。

对每条候选记忆输出 JSON 数组，每个元素包含：
- "fact": 一句简洁的原子事实（不含主观推测）
- "causes": 因果链（如 "A 引发 B"，没有则为空字符串）
- "level": L0（长期偏好/关系/身份/重要资料）或 L1（一般资料/近期上下文）
- "emotion": 情绪标签（如 joy / sadness / anger / surprise / fear / neutral）
- "intensity": 0~1 之间的情感强度
- "relation": 相对历史的 "causes" / "update" / "contradict" / "extend"（无则为空）

【重要度判定】用户主动填写的描述通常代表其关注点，优先视为 L0；解析出的细节多为 L1。

【硬性原则：禁止基于文件名编造内容】
如果『内容解析』以占位符开头（如「[音频转录失败]」、「[视频关键帧抽取失败]」、
「[图片描述失败]」、「[文档解析失败]」），说明系统未能成功解析文件实际内容。
此时：
  - 不要基于文件名/扩展名推测主题或具体事实（例如不要把"拉师傅来我家的三年.mp3"
    解读成"宠物相处三年的回忆"，那是幻觉）。
  - 若用户描述（『用户描述』字段）非空 → 仅基于用户描述提取事实，忽略占位内容。
  - 若用户描述也为空或为"（无）" → 输出空数组（不要为占位内容编造任何记忆）。
...
FILE_MEMORY_EXTRACT_USER_TEMPLATE = """文件名：{file_name}
文件类型：{modality}
用户描述：{desc}
内容解析：{parsed_content}
...
如果内容解析是占位文本（如"[音频转录失败] xxx"），仅依据用户描述提取；若用户描述为空，返回空数组。"""
